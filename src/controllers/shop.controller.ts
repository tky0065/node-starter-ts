...
export const createShop: RequestHandler = async (
    ...
    try {
        ...
        if (existingShop) { 
            res.status(StatusCodes.CONFLICT).json({ error: `Shop with this slug ${slug} already exist`, data: null });
        }
...
export const deleteShop: RequestHandler = async (
    ...
    try {
        ...
        res.status(StatusCodes.OK).send({message: `Shop with id ${id} deleted successfully`, error: null});
    } catch (error) {
        console.error(error);
        res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ error: `Internal Server Error ${error}` });
    }
...
export const getShopAttendants: RequestHandler = async (
    ...
    try {
        ...
        res.status(StatusCodes.OK).json({ data: attendants ,error: null });
    } catch (error) {
        console.error(error);
        res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ error: `Internal Server Error ${error}` });
    }
